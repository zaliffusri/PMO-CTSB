...
function notificationsTableMissing(error) {
  const msg = String(error?.message || '');
  return /schema cache|Could not find|does not exist|PGRST204|notifications_app/i.test(msg);
}

let ensuringNotificationsApp = null;

/** Best-effort DDL so notifications_app exists on older production DBs. */
export async function ensureNotificationsAppTable() {
  if (ensuringNotificationsApp) return ensuringNotificationsApp;
  ensuringNotificationsApp = (async () => {
    try {
      ...
      console.warn('ensureNotificationsAppTable failed:', e?.message || e);
      return false;
    } finally {
      ensuringNotificationsApp = null;
    }
  })();
  return ensuringNotificationsApp;
}

function attachmentsTableMissing(error) {
  const msg = String(error?.message || '');
  return /attachments_app/i.test(msg)
    && /schema cache|Could not find|does not exist|PGRST204|PGRST205/i.test(msg);
}

let ensuringAttachmentsApp = null;

/** Best-effort DDL so attachments_app exists on older production DBs. */
export async function ensureAttachmentsAppTable() {
  if (ensuringAttachmentsApp) return ensuringAttachmentsApp;
  ensuringAttachmentsApp = (async () => {
    try {
      const { getPgPool } = await import('./pgPool.js');
      const pool = getPgPool();
      if (!pool) return false;
      ...
      console.warn('ensureAttachmentsAppTable failed:', e?.message || e);
      return false;
    } finally {
      ensuringAttachmentsApp = null;
    }
  })();
  return ensuringAttachmentsApp;
}

export function attachmentsAppMissingMessage() {
  return (
    'attachments_app table is missing. Run migration '
    + '`20260911150000_ensure_attachments_app.sql` '
    + '(or set SUPABASE_DB_URL / SUPABASE_DB_PASSWORD so the API can create it).'
  );
}
...
export async function insertNotificationRemote(row) {
  ...
  const payload = {
    ...
    link: row.link || null,
    entity_type: row.entity_type || null,
    entity_id: row.entity_id != null && row.entity_id !== '' && Number.isFinite(Number(row.entity_id))
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (error && notificationsTableMissing(error) && !ensured) {
      ensured = true;
      const ok = await ensureNotificationsAppTable();
      if (ok) continue;
      throw new Error(
        'notifications_app table is missing. Run migration '
          + '`20260910170000_ensure_notifications_app.sql` '
          + '(or set SUPABASE_DB_URL so the API can create it).',
      );
    }

    if (error) {
      // Insert may succeed without a returning row under some RLS setups — fall back to PG.
      if (/0 rows|PGRST116|Cannot coerce/i.test(String(error.message || ''))) {
        const viaPg = await insertNotificationViaPg(payload);
        if (viaPg) return viaPg;
      }
      throw error;
    }

    // No error but empty representation — try direct Postgres insert.
    const viaPg = await insertNotificationViaPg(payload);
    if (viaPg) return viaPg;
...
async function insertNotificationViaPg(payload) {
  try {
    const { getPgPool } = await import('./pgPool.js');
    const pool = getPgPool();
    if (!pool) return null;
    const { rows } = await pool.query(
      `insert into public.notifications_app
        (user_id, type, title, body, link, entity_type, entity_id, read_at, created_at)
    ...
    return normalizeNotificationRow(rows[0] || null);
  } catch (e) {
    if (notificationsTableMissing(e)) {
      const ok = await ensureNotificationsAppTable();
      if (!ok) return null;
      return insertNotificationViaPg(payload);
    }
    console.warn('insertNotificationViaPg failed:', e?.message || e);
    return null;
  }
}
