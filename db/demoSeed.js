...
async function seedCalendarActivities(store, ctx) {
  const {
    p1, p2, p3, p4, p5, p6,
  ...
  const busyDay = -4;
  await add({ person_id: p1, project_id: projPortal, type: 'meeting', title: 'Daily stand-up', location: 'MSC Office', start_at: isoAt(busyDay, 9, 0), end_at: isoAt(busyDay, 9, 15) });
  await add({ person_id: p2, project_id: projPortal, type: 'meeting', title: 'CR review with client', location: 'DBKL HQ', start_at: isoAt(busyDay, 10, 0), end_at: isoAt(busyDay, 11, 0) });
  await add({ person_id: p3, project_id: projApi, type: 'meeting', title: 'API security review', location: 'MSC Office', start_at: isoAt(busyDay, 11, 30), end_at: isoAt(busyDay, 12, 30) });
  await add({ person_id: p4, project_id: projPortal, type: 'uat', title: 'UAT defect triage', location: 'MSC Office', start_at: isoAt(busyDay, 14, 0), end_at: isoAt(busyDay, 15, 30) });
  await add({ person_id: p5, project_id: projPortal, type: 'training', title: 'End-user training — DBKL clerks', location: 'DBKL HQ', start_at: isoAt(busyDay, 15, 30), end_at: isoAt(busyDay, 17, 0) });
...
async function ensureUser(store, { name, email, role, password }) {
 ...
 */
export async function runRichDemoSeed(store) {
  const adminId = await ensureUser(store, {
    name: 'Admin User',
    email: 'admin@pmo.local',
    role: 'admin',
    password: 'admin123',
  });
  const pmoId = await ensureUser(store, {
    name: 'Nurul PMO',
    email: 'pmo@pmo.local',
    role: 'pmo',
    password: 'pmo123',
  });
  await ensureUser(store, {
  ...
  const mohId = await store.findOrCreateClient('Ministry of Health');
  const dbklId = await store.findOrCreateClient('DBKL');
  const mbsaId = await store.findOrCreateClient('MBSA (Shah Alam)');
  const uthmId = await store.findOrCreateClient('UTHM');

  const p1 = await store.addPerson({ name: 'Ahmad Rizal', email: 'ahmadrizal@company.com', role: 'Developer' });
  const p2 = await store.addPerson({ name: 'Siti Nur', email: 'sitinu@company.com', role: 'Business Analyst' });
  const p3 = await store.addPerson({ name: 'Lee Wei Ming', email: 'leeweiming@company.com', role: 'Developer' });
  const p4 = await store.addPerson({ name: 'Priya Sharma', email: 'priyasharma@company.com', role: 'QA Engineer' });
  const p5 = await store.addPerson({ name: 'John Tan', email: 'johntan@company.com', role: 'Tech Lead' });
  const p6 = await store.addPerson({ name: 'Farah Ilyana', email: 'farah@company.com', role: 'Support Engineer' });

  const projPortal = await store.addProject({
  ...
  await store.setProjectClients(projApi, [mohId]);

  const projMaint = await store.addProject({
  ...
  await store.setProjectClients(projMaint, [dbklId]);

  const projMigration = await store.addProject({
  ...
  await store.setProjectClients(projMigration, [uthmId]);

  const projTender = await store.addProject({
  ...
  await store.setProjectClients(projTender, [dbklId]);
  ...
  for (const { id, classification } of projects) {
    ...
    await store.initProjectPhasesFromTemplate(id, template);
  }
  ...
  if (maintOnboarding) {
    await store.updateProjectPhase(maintOnboarding.id, {
      status: 'completed',
  ...
  if (maintPeriod) {
    await store.updateProjectPhase(maintPeriod.id, {
      status: 'in_progress',
      progress_percent: 35,
  ...
  const projMaint2025 = await store.addProject({
  ...
  await store.setProjectClients(projMaint2025, [mbsaId]);
  await store.initProjectPhasesFromTemplate(projMaint2025, templateForClassification('Maintenance & Support'));
  for (const ph of store.project_phases.filter((p) => p.project_id === projMaint2025)) {
    if (ph.payment_status === 'not_applicable') {
      await store.updateProjectPhase(ph.id, { status: 'completed', progress_percent: 100 });
      continue;
    }
    await store.updateProjectPhase(ph.id, {
      status: 'completed',
      progress_percent: 100,
  ...
  await store.initProjectPhasesFromTemplate(projPortal, templateForClassification('Data Migration'), wpData);
  ...
  if (ursPhase) {
    await store.updateProjectPhase(ursPhase.id, {
      status: 'completed',
      progress_percent: 100,
      completed_date: dayOffset(-45),
  ...
  if (devPhase) {
    await store.updateProjectPhase(devPhase.id, {
      status: 'in_progress',
      progress_percent: 62,
  ...
  if (uatPhase) {
    await store.updateProjectPhase(uatPhase.id, {
  ...
  if (payGoLive) {
    await store.updateProjectPhase(payGoLive.id, {
      status: 'completed',
      progress_percent: 100,
  ...
  if (apiUat) {
    await store.updateProjectPhase(apiUat.id, {
      status: 'completed',
      progress_percent: 100,
  ...
  await store.addAssignment({ project_id: projPortal, person_id: p5, role_in_project: 'Project Lead', allocation_percent: 50 });
  await store.addAssignment({ project_id: projPortal, person_id: p1, role_in_project: 'Developer', allocation_percent: 80 });
  await store.addAssignment({ project_id: projPortal, person_id: p2, role_in_project: 'BA', allocation_percent: 60 });
  await store.addAssignment({ project_id: projPortal, person_id: p4, role_in_project: 'QA', allocation_percent: 40 });
  await store.addAssignment({ project_id: projApi, person_id: p3, role_in_project: 'Lead Developer', allocation_percent: 90 });
  await store.addAssignment({ project_id: projApi, person_id: p1, role_in_project: 'Developer', allocation_percent: 30 });
  await store.addAssignment({ project_id: projMaint, person_id: p6, role_in_project: 'Support', allocation_percent: 70 });
  await store.addAssignment({ project_id: projMaint, person_id: p2, role_in_project: 'Coordinator', allocation_percent: 20 });
  await store.addAssignment({ project_id: projMigration, person_id: p2, role_in_project: 'Analyst', allocation_percent: 50 });
  await store.addAssignment({ project_id: projTender, person_id: p5, role_in_project: 'Solution Architect', allocation_percent: 40 });

  const gPortal = await store.addProjectTask({
    project_id: projPortal,
  ...
  const issueLogin = await store.addIssue({
  ...
  const issueCr = await store.addIssue({
  ...
  const issueSupport = await store.addIssue({
  ...
  const issueData = await store.addIssue({
  ...
  await store.updateBacklog(blBug, { task_id: taskFromBacklog, status: 'in_progress' });
  ...
  await seedCalendarActivities(store, {
    p1, p2, p3, p4, p5, p6,
    projPortal, projApi, projMaint, projMigration, projTender,
  });

  const bulkStats = await seedBulkVolumeData(store, {
    adminId,
    pmoId,
