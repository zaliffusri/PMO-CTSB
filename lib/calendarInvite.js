...
export function buildActivityIcs({
  ...
  // Keep SUMMARY identical on cancel so Outlook/Teams match the original invite by UID.
  const summary = title || 'Activity';
  const descParts = [
    isCancel ? 'This activity has been cancelled in PMO CTSB.' : null,
  ...
  // Include the same RRULE on cancel so Outlook/Teams remove the whole daily series.
  ...
  const lines = [
    ...
    isCancel ? 'TRANSP:TRANSPARENT' : 'TRANSP:OPAQUE',
    'CLASS:PUBLIC',
    isCancel ? 'X-MICROSOFT-CDO-BUSYSTATUS:FREE' : 'X-MICROSOFT-CDO-BUSYSTATUS:BUSY',
  ...
  if (orgEmail) {
    ...
    // Outlook/Teams require an explicit mailto: organizer matching the From address.
  ...
  for (const person of party) {
    ...
    if (isCancel) {
      // DECLINED + CANCEL is what Outlook expects for meeting cancellation processing.
      lines.push(
