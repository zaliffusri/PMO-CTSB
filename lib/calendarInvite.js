...
export function buildActivityIcs({
  ...
  // UTC form is the most reliable for Outlook / Teams METHOD:REQUEST + CANCEL matching.
  ...
  for (const person of party) {
    ...
    if (isCancel) {
      lines.push(
        `ATTENDEE;CN=${cn};ROLE=REQ-PARTICIPANT;PARTSTAT=DECLINED;RSVP=FALSE:mailto:${person.email}`,
      );
    } else {
      lines.push(
        `ATTENDEE;CN=${cn};ROLE=REQ-PARTICIPANT;PARTSTAT=NEEDS-ACTION;RSVP=TRUE:mailto:${person.email}`,
      );
    }
  }
