...
export const createProjectSchema = z.object({
  name: z.string().trim().min(1, 'name is required'),
  // Optional: older clients may omit it — route derives from name when missing.
  short_code: z.preprocess(
    ...
    z
      ...
      .max(16, 'short code must be at most 16 characters')
      .regex(/^[A-Za-z0-9]+$/, 'short code must be letters and numbers only')
      .transform((v) => v.toUpperCase())
      .optional(),
  ),
  description: optionalNullableString,
  classification: optionalNullableString,
  engagement_type: optionalNullableString,
  status: z.string().optional(),
  start_date: optionalNullableString,
  end_date: optionalNullableString,
  tags: z.array(z.string()).optional(),
  client_ids: z.array(z.number().int().positive()).max(1).optional(),
