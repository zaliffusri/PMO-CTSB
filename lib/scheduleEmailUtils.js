...
export async function resolveScheduleRecipients(store, { mode, customEmails = [], activityRows = [] }) {
  ...
  if (mode === 'assignees') {
    ...
    for (const row of activityRows) {
      for (const pid of row.person_ids || []) {
        const u = await store.findUserById(pid);
        add(u?.email);
        if (!u?.email && u?.name) {
  ...
  people.forEach((p) => add(p.email));
  users.filter((u) => u.active !== false).forEach((u) => add(u.email));
  return [...emails];
}
