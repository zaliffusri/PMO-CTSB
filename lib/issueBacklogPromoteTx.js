  ...
  return withTransaction(async (client) => {
    ...
    if (pid) {
      const { rows: projectRows } = await client.query(
        `SELECT short_code FROM public.projects WHERE id = $1 LIMIT 1`,
        [pid],
      ).catch(() => ({ rows: [] }));
      projectShortCode = String(projectRows?.[0]?.short_code || '').trim() || null;
      if (!projectShortCode) {
        ...
        projectShortCode = String(clientRows?.[0]?.short_code || '').trim() || null;
      }
      if (!projectShortCode && issue.client_id) {
        const { rows: byClient } = await client.query(
          `SELECT short_code FROM public.clients WHERE id = $1 LIMIT 1`,
          [issue.client_id],
        ).catch(() => ({ rows: [] }));
        projectShortCode = String(byClient?.[0]?.short_code || '').trim() || null;
      }
    }
