    ...
    if (variant === 'cancelled') {
      ...
      for (const existingId of existingIds) {
        ...
        try {
          ...
          console.warn(`msGraph: delete failed for ${email}:`, e.message);
        }
      }

      if (deleted > 0) return { ok: true, action: 'deleted', email, deleted };
      if (marked > 0) return { ok: true, action: 'marked_canceled', email, marked };
      return { ok: false, reason: 'delete_failed', email };
    }
