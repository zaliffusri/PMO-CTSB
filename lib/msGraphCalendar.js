...
import { resolveMsGraphConfig } from './msGraphConfig.js';

/**
 * Optional Microsoft Graph calendar sync so Teams / Outlook calendars
 * are created, updated, and marked Canceled when PMO activities change.
 *
 * Configure in Settings → Teams calendar, or env:
 *   MS_GRAPH_TENANT_ID, MS_GRAPH_CLIENT_ID, MS_GRAPH_CLIENT_SECRET
 * Azure app needs application permission Calendars.ReadWrite (admin consent).
 */
...
export function isMsGraphConfigured() {
  return resolveMsGraphConfig().configured;
}

export function invalidateMsGraphCache() {
  cachedToken = null;
  cachedTokenExp = 0;
  cachedTokenKey = '';
}
...
async function getAccessToken() {
  ...
  if (!cfg.configured) throw new Error('Microsoft Graph is not configured');

  const key = `${cfg.tenantId}|${cfg.clientId}|${cfg.clientSecret}`;
  const now = Date.now();
  if (cachedToken && cachedTokenKey === key && cachedTokenExp > now + 60_000) return cachedToken;
...
/** Admin test: fetch a token to verify credentials + consent. */
export async function testMsGraphConnection() {
  ...
  try {
    ...
    return { ok: true, source: resolveMsGraphConfig().source };
  } catch (e) {
    return { ok: false, reason: e.message || 'token_failed' };
  }
}
...
function buildGraphEventPayload({
  ...
  const event = {
    ...
    // Do not set isCancelled — Graph treats that as read-only; use subject + free status instead.
    // Do not call POST .../cancel — that moves the event to Deleted Items.
    showAs: canceled ? 'free' : 'busy',
    isReminderOn: !canceled,
    categories: canceled ? ['Canceled'] : undefined,
  };
...
function eventMatchesTitle(ev, title) {
  const sub = String(ev?.subject || '').trim().toLowerCase();
...
async function findEventIds(userEmail, calendarUid, startAt, endAt, title = '') {
  const events = await listEventsNear(userEmail, startAt, endAt);
  const ids = [];
  const seen = new Set();
  for (const ev of events) {
    if (!ev?.id || seen.has(ev.id)) continue;
    if (eventMatchesUid(ev, calendarUid) || eventMatchesTitle(ev, title)) {
      seen.add(ev.id);
      ids.push(ev.id);
    }
  }
...
export async function syncMsGraphRecipient({
  ...
  try {
    if (variant === 'cancelled') {
      const existingIds = await findEventIds(email, calendarUid, startAt, endAt, title);
      if (!existingIds.length) {
        return { ok: true, action: 'not_found', email };
      }

      // Keep the event on Teams/Outlook — only mark it Canceled (do not delete).
      let marked = 0;
      ...
      if (marked > 0) return { ok: true, action: 'marked_canceled', email, marked };
      return { ok: false, reason: 'cancel_failed', email };
    }
