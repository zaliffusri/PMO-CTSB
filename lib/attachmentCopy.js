...
export async function copyAttachments(store, fromType, fromId, toType, toId) {
  const source = await store.listAttachments(fromType, fromId);
  if (!source.length) return 0;
  const existing = await store.listAttachments(toType, toId);
