 ...
 * @returns {number} rows created
 */
export function repairLegacyClientContacts(store, clientId) {
  ...
  for (const cc of [...contacts]) {
    ...
    store.deleteClientContact(cc.id);
...
export function repairAllLegacyClientContacts(store) {
  let clientsTouched = 0;
  let contactsCreated = 0;
  for (const client of store.clients || []) {
    const n = repairLegacyClientContacts(store, client.id);
    if (n > 0) {
      clientsTouched += 1;
      contactsCreated += n;
    }
