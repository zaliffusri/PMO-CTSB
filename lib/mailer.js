...
async function sendActivityCalendarEmail({
  ...
  const mail = {
    ...
    text: built.text,
    html: built.html,
    // Helps Outlook / Microsoft 365 / Teams treat the message as a meeting invite.
    headers: {
  ...
  if (ics) {
    const filename = isCancel ? 'invite-cancel.ics' : 'invite.ics';
    const calendarType = `text/calendar; charset=UTF-8; method=${icsMethod}`;
    mail.icalEvent = {
      filename,
      method: icsMethod,
      content: ics,
    ...
    mail.attachments = [
      {
        filename: isCancel ? 'PMO-CTSB-cancel.ics' : 'PMO-CTSB-invite.ics',
        content: ics,
        contentType: calendarType,
