...
async function sendActivityCalendarEmail({
  ...
  // Outlook / Teams auto-cancel requires a single text/calendar part with method=CANCEL
  // matching the ICS METHOD. Do not also attach a second .ics — that becomes
  // "not supported calendar message.ics" and the Teams calendar is left unchanged.
  const mail = {
    from,
    to,
    // Outlook looks for the US spelling "Canceled:" to treat this as a meeting cancellation.
    subject: isCancel ? `Canceled: ${title || 'Activity'}` : built.subject,
    text: built.text,
    html: built.html,
    headers: {
