...
/** Safe uppercase token for backlog refs (letters/digits only). */
export function slugBacklogRefPart(value, fallback = 'X') {
...
/**
 * Backlog reference: MENU-PROJECTSHORTCODE-MODULE-0001
 * e.g. CUKAI-MBIP-CK-0001
 */
export function nextBacklogRefNo({
  backlogs = [],
  menu,
  projectShortCode,
  moduleCode = 'XXX',
} = {}) {
  const menuPart = slugBacklogRefPart(menu, 'GEN');
  const projectPart = slugBacklogRefPart(projectShortCode, 'PRJ');
  const modulePart = normalizeModuleCode(moduleCode);
  const prefix = `${menuPart}-${projectPart}-${modulePart}-`;
  const prefixUpper = prefix.toUpperCase();
  const nums = (backlogs || [])
    .map((b) => String(b?.ref_no || '').trim().toUpperCase())
    .filter((ref) => ref.startsWith(prefixUpper))
    .map((ref) => parseInt(ref.slice(prefixUpper.length), 10))
    .filter((n) => Number.isFinite(n) && n > 0);
  const next = nums.length ? Math.max(...nums) + 1 : 1;
  return `${prefix}${String(next).padStart(4, '0')}`;
}

/** @deprecated Prefer nextBacklogRefNo — kept for callers that only pass module. */
export function nextModuleBacklogRef(backlogs = [], moduleCode = 'XXX', opts = {}) {
  return nextBacklogRefNo({
    backlogs,
    menu: opts.menu,
...
export function resolveBacklogRefForIssue(issue, backlogs = [], opts = {}) {
  ...
  return nextBacklogRefNo({
    ...
    menu: opts.menu || 'HD',
    projectShortCode: opts.projectShortCode,
    moduleCode: issue?.module_code || opts.moduleCode,
  });
}

async function resolveProjectShortCode(store, projectId, issue = null) {
  if (projectId != null && typeof store.findProjectById === 'function') {
    const project = await store.findProjectById(+projectId).catch(() => null);
    const projectCode = String(project?.short_code || '').trim();
    if (projectCode) return projectCode;
  }
  if (issue?.client_short_code) return String(issue.client_short_code).trim();
  if (issue?.client_id && typeof store.findClientById === 'function') {
    const client = await store.findClientById(issue.client_id).catch(() => null);
    if (client?.short_code) return String(client.short_code).trim();
  }
  if (projectId != null && typeof store.getClientsForProject === 'function') {
    const clients = await store.getClientsForProject(+projectId).catch(() => []);
...
export async function promoteIssueToBacklog(store, issueId, projectId, { createdByUserId = null, assigneePersonId = null } = {}) {
  ...
  const projectShortCode = await resolveProjectShortCode(store, projectId, issue);
  const refNo = resolveBacklogRefForIssue(issue, allBacklogs, { projectShortCode, menu: 'HD' });
  ...
  const backlogSaved = await store.addBacklog(payload);
  const backlogId = backlogSaved?.id ?? backlogSaved;
  ...
  const freshBacklogs = await store.listBacklogs();
  return {
    backlog: freshBacklogs.find((b) => Number(b.id) === Number(backlogId)),
    created: true,
    linked: true,
  };
}
