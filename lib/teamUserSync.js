...
/**
 * Roster visibility for Calendar/Team linked_only lists.
 * Prefers people.user_id; if PostgREST omits the column (stale schema cache),
 * fall back to email match against active users so the UI is not empty.
 */
export function isPersonVisibleOnLinkedRoster(person, users = []) {
  if (isPersonLinkedToUser(person, users)) return true;
  if (!person || Object.prototype.hasOwnProperty.call(person, 'user_id')) return false;
  const email = normalizeEmail(person.email);
  if (!email) return false;
  return (users || [])
    .filter((u) => u.active !== false)
    .some((u) => normalizeEmail(u.email) === email);
}
...
/** Linked roster via SQL (includes user_id even when table schema cache is stale). */
export async function listLinkedPeopleViaRpc() {
  const { requireSupabase } = await import('../db/runtime/query.js');
  const sb = requireSupabase();
  const { data, error } = await sb.rpc('list_linked_people');
  if (error) throw error;
...
export async function pruneOrphanPeople(store, { requireNoAssignments = true } = {}) {
  const people = await store.listPeople();
  // If PostgREST schema cache omits people.user_id, every row looks unlinked — do not wipe.
  if (people.length > 0 && !people.some((p) => Object.prototype.hasOwnProperty.call(p, 'user_id'))) {
    console.warn(
      'pruneOrphanPeople skipped: people.user_id missing from API rows (reload PostgREST schema cache)',
    );
    return [];
  }
  const orphans = await findOrphanPeople(store);
