...
export function userIdForPersonId(personId) {
  ...
  const person = store.people.find((p) => Number(p.id) === Number(personId));
 ...
 */
export function resolveAppUserId(rawId) {
  if (rawId == null || rawId === '') return null;
  const n = Number(rawId);
  if (!Number.isFinite(n)) return null;
  if (store.findUserById(n)) return n;
  return userIdForPersonId(n);
}
...
export async function notifyInApp(payload) {
  ...
  return store.addNotification(row);
}

export async function notifyPersonInApp(personId, payload) {
  const userId = resolveAppUserId(personId);
