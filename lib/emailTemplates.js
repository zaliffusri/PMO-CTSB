...
function calendarLinksHtml({ outlookUrl, cancelled = false } = {}) {
  if (cancelled) {
  ...
  return `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">
      <strong>Calendar invite included</strong> — Outlook / Teams should add this event automatically. Fallback: <a href="${escapeHtml(outlookUrl)}" style="color:#059669;font-weight:600;text-decoration:none;">Outlook</a>
    </p>`;
}

function calendarLinksText({ outlookUrl, cancelled = false } = {}) {
  if (cancelled) {
    return 'A calendar cancellation is attached — your calendar should remove this event automatically.';
  }
  const lines = [
    'A meeting invite is attached — Outlook / Teams should add this event automatically.',
  ];
...
export function buildActivityNotificationEmail({
  ...
  </table>${calendarLinksHtml({ outlookUrl: outlookCalendarUrl })}`;
  ...
  const text = [
    ...
    '',
    calendarLinksText({ outlookUrl: outlookCalendarUrl }),
    '',
