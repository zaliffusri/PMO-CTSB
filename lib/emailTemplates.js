...
function calendarLinksHtml({ googleUrl, outlookUrl, cancelled = false } = {}) {
  if (cancelled) {
    return `<p style="margin:16px 0 0;font-size:13px;color:#64748b;">A calendar cancellation (.ics) is attached — open it to remove this event from your calendar.</p>`;
  }
  if (!googleUrl && !outlookUrl) {
    return `<p style="margin:16px 0 0;font-size:13px;color:#64748b;">A calendar invite (.ics) is attached — open it to add this event at the same date and time.</p>`;
  }
  const links = [
    googleUrl
  ...
  return `<p style="margin:16px 0 0;font-size:14px;line-height:1.6;">
      <strong>Add to your calendar</strong> (same date &amp; time): ${links.join(' · ')}
      <br /><span style="font-size:12px;color:#64748b;">Or open the attached .ics invite in Apple / Outlook / other calendar apps.</span>
    </p>`;
}

function calendarLinksText({ googleUrl, outlookUrl, cancelled = false } = {}) {
  if (cancelled) return 'A calendar cancellation (.ics) is attached.';
  const lines = ['Add to your calendar (same date & time):'];
  if (googleUrl) lines.push(`Google Calendar: ${googleUrl}`);
  if (outlookUrl) lines.push(`Outlook: ${outlookUrl}`);
  lines.push('Or open the attached .ics invite.');
  return lines.join('\n');
}
...
export function buildActivityUpdatedEmail({
  ...
  const html = wrapEmailLayout({
    ...
    preheader: `Updated: ${title} — ${startAt} → ${endAt}`,
    orgName,
    bodyHtml: greeting + details,
    ctaLabel: 'Open calendar',
    ctaUrl: process.env.APP_URL ? `${process.env.APP_URL.replace(/\/$/, '')}/calendar` : undefined,
...
export function buildActivityCancelledEmail({
  ...
  const html = wrapEmailLayout({
    ...
    preheader: `Cancelled: ${title} — ${startAt}`,
    orgName,
    bodyHtml: greeting + details,
    ctaLabel: 'Open calendar',
    ctaUrl: process.env.APP_URL ? `${process.env.APP_URL.replace(/\/$/, '')}/calendar` : undefined,
  ...
  return { html, text, subject: `PMO CTSB: Activity cancelled — ${title}` };
}
